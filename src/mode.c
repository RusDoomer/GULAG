...
void improve(int shuffle) {
    ...
    char filename[256];
    if (shuffle) {
        snprintf(filename, sizeof(filename), "greedy1swap_log_%s_shuffle_%d_%d_%s.log", weight_name, threads, repetitions, timestamp);
    } else {
        snprintf(filename, sizeof(filename), "greedy1swap_log_%s_%s_%d_%d_%s.log", weight_name, layout_name, threads, repetitions, timestamp);
    }
